...
class CmdLineParser(object):
	def __init__(self):
		self.parser = OptionParser(usage='usage: python3 perfprof.py [options] cvsfile.csv outputfile.pdf')
		# default options
...
def readTable(fp, delimiter):
	...
	...
	"""
	firstline = fp.readline().strip().split(",")
	ncols = getint(firstline[0])
...
def main():
	...
	print(opt)
	# read data
	_, cnames, data = readTable(open(opt.input, 'r'), opt.delimiter)
